...
cpdef int io_uring_queue_init_sqpoll(unsigned int entries, io_uring ring, unsigned int sq_thread_cpu) nogil:
    if ring.active:
        raise RuntimeError('`io_uring_queue_init_sqpoll(ring)` already initialized!')
    cdef __io_uring_params p
    memset(&p, 0, sizeof(__io_uring_params))
    p.flags = 1 << 1  # IORING_SETUP_SQPOLL
    p.sq_thread_cpu = sq_thread_cpu
    p.sq_thread_idle = 30000  # milliseconds
    ring.active = True
    return trap_error(__io_uring_queue_init_params(entries, &ring.ptr, &p))
