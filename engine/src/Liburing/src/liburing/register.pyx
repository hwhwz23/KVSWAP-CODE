...
cpdef int set_iowq_affinity(io_uring ring, int[:] cpus) nogil:
    cdef cpu_set_t mask
    cdef int cpu_id
    CPU_ZERO(&mask)
    for cpu_id in cpus:
        if cpu_id < 0 or cpu_id >= 1024:
            raise ValueError(f"cpu_id {cpu_id} is out of range")
        CPU_SET(cpu_id, &mask)
    return trap_error(__io_uring_register_iowq_aff(&ring.ptr, sizeof(mask), &mask))
...
cpdef int io_uring_register_iowq_max_workers(io_uring ring, unsigned int[:] values) nogil:
    cdef unsigned int arr[2]
    arr[0] = values[0]
    arr[1] = values[1]
    return trap_error(__io_uring_register_iowq_max_workers(&ring.ptr, &arr[0]))
