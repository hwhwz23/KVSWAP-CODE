...
class LLM:
    ...
    def init_kv_cache(self, sparse_budget: int, rank: int, chunk_size: int, config, **kwargs):
        ...
        if self.attn_mode == 'full':
            self.kv_cache = KV_Cache(config, max_length=self.max_length, device=device, dtype=self.dtype, batch_size=self.batch_size)
        elif self.attn_mode.lower() == 'shadowkv':
            self.kv_cache = ShadowKVCache(config, max_length=self.max_length, device=device, dtype=self.dtype, 
            ...
            self.kv_cache = ShadowKVCache_CPU(config, max_length=self.max_length, device=device, dtype=self.dtype, batch_size=self.batch_size, sparse_budget=sparse_budget, rank=rank, chunk_size=chunk_size)
        else:
    ...
    def inference(self,
        ...
        if input_ids is None:
            assert input_embs is not None, "input_ids and input_embs cannot be both None"
            assert input_embs.shape[0] == self.batch_size, f"input_embs batch size {input_embs.shape[0]} does not match model batch size {self.batch_size}"
            hidden_states = input_embs
        else:
            hidden_states = F.embedding(input_ids, self.embed_tokens)
        
        # print("position_ids shape:", position_ids.shape, flush=True)
        if position_ids.dim() == 3:
            position_embeddings = self.rotary_emb(hidden_states, position_ids)
            # print(position_embeddings[0].shape, flush=True)
            # print(position_embeddings[1].shape, flush=True)
            for idx in range(self.num_layers):
                hidden_states = self.layer_compute(self.layers[idx], idx, hidden_states, position_ids_emb=position_embeddings)
        else:
            for idx in range(self.num_layers):
    ...
    @torch.inference_mode()
    def prefill(self, input_ids: torch.LongTensor = None, 
                        input_embs: torch.Tensor = None, **kwargs):
        self.kv_cache.clear()
        if input_ids is not None:
            logits = self.inference(input_ids=input_ids, input_embs=None, 
                                    position_ids=self.get_ctx(input_ids))
            ...
            assert input_embs is not None, "input_ids and input_embs cannot be both None"
            self.cached_cos_sin_cache = None
            logits = self.inference(input_ids=None, input_embs=input_embs, 
                                    position_ids=kwargs['position_ids'])
            assert self.kv_cache.get_kv_len() == input_embs.shape[-2], f"KV length mismatch, got {self.kv_cache.get_kv_len()}, expected {input_embs.shape[-2]}"
        return logits
        ...
        if isinstance(self.kv_cache, KV_Cache):
            ...
            if q_len >= 2*1024: # prefill
                ...
                key_states = self.kv_cache.get_key_cache(layer_idx=layer_idx, position_ids=position_ids, 
                                                         rope_func=self.apply_rotary_pos_emb_single, cos_sin_cache=self.cos_sin_cache if hasattr(self, 'cos_sin_cache') else None)
    ...
    @torch.inference_mode()
    def generate(self, input_ids: torch.Tensor = None, input_embs: torch.Tensor = None, gen_len: int = 256, temperature: float = 0.0, 
                 top_p: float = 0.9, top_k :int = 50, verbose: bool = False, benchmark: bool = False, cont: bool = False, **kwargs):
        ...
        if input_ids is not None:
            ...
            assert type(input_embs) == torch.Tensor, f"input_embs must be a torch.Tensor, got {type(input_embs)}"
            input_size = input_embs.size(1)

        # prefill
        if cont == False:
            if input_size > self.max_length:
                raise ValueError(f"Input length must be less than {self.max_length}, but got {input_size}")
            logits = self.prefill(input_ids=input_ids, input_embs=input_embs, **kwargs)
        else:
            if input_size + self.kv_cache.get_kv_len() >= self.max_length:
                raise ValueError(f"Input length must be less than {self.max_length}, but got {input_size}")
        ...
        while n < gen_len:
            ...
            if input_embs is not None:
                ...
                rope_deltas = kwargs['rope_deltas']
                cache_position = kwargs['cache_position']
                delta = (cache_position[0] + rope_deltas).to(self.device)
            ...
            logits = self.inference(input_ids=next_token, position_ids=position_ids)
            next_token = sample_token(logits[:, -1, :], temperature=temperature, top_p=top_p, top_k=top_k)
        ...
        if input_embs is not None:
            ...
            batch_size = input_embs.size(0)
            rope_deltas = kwargs['rope_deltas']
            delta = (position_ids[0,0] + rope_deltas).to(self.device)
            position_ids = torch.arange(seq_length, device=self.device)
            position_ids = position_ids.view(1, -1).expand(batch_size, -1)
            delta = delta.repeat_interleave(batch_size // delta.shape[0], dim=0)
            position_ids = position_ids.add(delta)
        ...
        self.inference(input_ids=next_token, position_ids=position_ids)
