...
class ShadowKVCache:
    ...
    def __init__(self, 
        ...
        **kwargs
        ...
        self.sparse_budget = (int(sparse_budget) + chunk_size -1 ) // chunk_size * chunk_size # make sure sparse_budget is a multiple of chunk_size
        self.chunk_size = chunk_size
        self.rank = rank
        # self.local_chunk = 4
        if 'local_chunk' in kwargs and kwargs['local_chunk'] is not None:   
            self.local_chunk = kwargs['local_chunk']
        else:
            self.local_chunk = int(round(4*8/chunk_size))
        # self.outlier_chunk = 48
        if 'outlier_chunk' in kwargs and kwargs['outlier_chunk'] is not None:   
            self.outlier_chunk = kwargs['outlier_chunk']
        else:
            self.outlier_chunk = int(round(48*8/chunk_size))
    ...
    def prefill_kv_cache(self,
        ...
        # print(self.prefill_local, self.chunks, self.chunk_size, self.select_sets, incoming, self.outlier_chunk, self.sparse_budget, flush=True)
        
        self.k_cache_buffer[layer_idx][:, :, :self.prefill_local].copy_(key_states_roped[:, :, key_states_roped.shape[2]-self.prefill_local:])
        self.v_cache_buffer[layer_idx][:, :, :self.prefill_local].copy_(new_v_cache[:, :, new_v_cache.shape[2]-self.prefill_local:])
